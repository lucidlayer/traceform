...
let lastServerStatus = 'idle'; // Include idle state
...
async function checkServerStatus() {
    ...
    let currentStatus = 'down'; // Assume down unless proven otherwise
    try {
...
function connectWebSocket() {
                ...
                if (tabs && tabs.length > 0 && tabs[0].id) {
                    ...
                    const sendMessageWithRetry = (attempt = 1) => {
                        chrome.tabs.sendMessage(targetTabId, message, (response) => {
                            if (chrome.runtime.lastError) {
                                console.error(`Attempt ${attempt}: Error sending message to tab ${targetTabId}:`, chrome.runtime.lastError.message);
                                if (attempt < 3) { // Retry up to 2 times (total 3 attempts)
                                    console.log(`Retrying message send in 200ms...`);
                                    setTimeout(() => sendMessageWithRetry(attempt + 1), 200);
                                }
                                else {
                                    // Failed after retries, notify panel
                                    broadcastToDevtools({ type: "error", message: `Failed to send message to content script after ${attempt} attempts. Try refreshing the target page. (${chrome.runtime.lastError.message})` });
                                }
                            }
                            else {
                                // console.log(`Attempt ${attempt}: Message sent successfully. Response:`, response); // For debugging
                            }
                        });
                    };
                    sendMessageWithRetry(); // Initial attempt
                }
                else {
    ...
    socket.onclose = (event) => {
        console.log(`WebSocket connection closed. Code: ${event.code}, Reason: ${event.reason}. Attempting reconnect in ${reconnectInterval / 1000}s.`);
