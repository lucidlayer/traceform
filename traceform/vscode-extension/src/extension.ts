...
export async function activate(context: vscode.ExtensionContext) { // Make activate async
	...
	const findInUICommand = vscode.commands.registerCommand('traceform.findInUI', async () => { // Make async for potential source map loading
		...
		// Attempt to find the function/class declaration enclosing the cursor
		let componentName: string | null = null;
		for (let line = position.line; line >= 0; line--) {
			const lineText = document.lineAt(line).text;
			// Basic regex for function/class declaration (can be improved)
			const match = lineText.match(/(?:function|class)\s+([A-Z][a-zA-Z0-9_]*)/);
			if (match && match[1]) {
				componentName = match[1];
				break;
			}
			// Basic regex for const MyComponent = (...) => { ... }
			const arrowMatch = lineText.match(/(?:const|let|var)\s+([A-Z][a-zA-Z0-9_]*)\s*=/);
		...
		if (!componentName) {
			// Fallback to selected text if inference fails
			if (!selection.isEmpty) {
				componentName = document.getText(selection).trim();
				// Basic validation if using selection
				if (!/^[A-Z]/.test(componentName)) {
					vscode.window.showWarningMessage(`Selected text "${componentName}" doesn't look like a component name.`);
					return;
				}
			} else {
				vscode.window.showWarningMessage('Could not infer component name near cursor. Please select the component name.');
				return;
			}
		}
		// --- End Component Name Inference ---
		...
		// Component name is now inferred above
