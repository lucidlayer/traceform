...
*   **Unit Tests (Jest):** `babel-plugin/__tests__/index.test.ts`
    *   **Case 1.1:** Injects `data-traceform-id` attribute into simple functional components.
    *   **Case 1.2:** Injects `data-traceform-id` attribute into simple class components.
    ...
    *   **Case 1.7:** Handles components with existing `data-traceform-id` attributes (should overwrite or skip based on implementation choice - *verify chosen behavior*).
    *   **Case 1.8:** Handles components with no explicit return (e.g., `() => <div />`).
*   **Manual Build Check:**
    *   **Case 1.9:** Build the `example-react-app` and inspect the output bundle or running app DOM to confirm `data-traceform-id` attributes are present on expected components (#EX_BTN, #EX_AVATAR, etc.).

#### 2. Bridge Server (Integrated into #VSC_EXT)
*   **Unit/Integration Tests (Potentially Jest or manual - within VS Code Extension context):**
    *   **Case 2.1:** Server starts without errors on the specified port (9901) when VS Code extension activates.
...
*   **Content Script (#EXT_CONTENT):**
    ...
    *   **Case 3.7:** On receiving a `HIGHLIGHT_COMPONENT` message, queries the DOM for elements matching `[data-traceform-id="ComponentName"]`.
    ...
    *   **Case 3.11 (Dynamic Content):** If a MutationObserver is used, verify it detects newly added components with `data-traceform-id` attributes.
...
*   **Extension Logic (#VSC_EXT):**
    *   **Case 4.1:** "Traceform: Find Component in UI" command appears in the editor context menu.
...
*   **Manual Verification:**
    ...
    *   **Case 5.2:** All custom components (#EX_BTN, #EX_AVATAR, etc.) rendered in the browser have the `data-traceform-id` attribute with the correct component name value (verified via browser dev tools).
    *   **Case 5.3:** App functions normally (buttons clickable, navigation works).

#### 6. End-to-End Integration (Highlight Flow)
*   **Manual Test Protocol:**
    *   **(Removed) Step 6.1:** Bridge server starts automatically with VS Code extension.
    *   **Step 6.2:** Build and load the Traceform: UI Mapping Browser Extension (unpacked).
    *   **Step 6.3:** Install the Traceform for VS Code Extension from Marketplace or VSIX.
    *   **Step 6.4:** Build and run the Example React App (`npm run dev`).
    *   **Step 6.5:** Open the Example React App in the browser (ensure the browser extension is active).
    *   **Step 6.6:** In VS Code, open a component file (e.g., `Button.tsx`). Right-click on the component name or within its definition and select "Traceform: Find Component in UI".
    *   **Expected 6.6:** The corresponding Button component(s) in the browser should be highlighted by the overlay. Verify console logs in Browser Extension background/content scripts, and VS Code Output Panel ("Traceform") for correct message flow.
...
### Acceptance Criteria
- **Babel Plugin:** Correctly injects `data-traceform-id` attributes into all applicable React components in the example app build. Unit tests pass.
- **Bridge Server (Integrated):** Successfully relays `HIGHLIGHT_COMPONENT` messages between connected clients without loss or corruption. Handles connections/disconnections robustly.
...
- **VS Code Extension:** Reliably extracts component names and sends correct `HIGHLIGHT_COMPONENT` messages to the Bridge Server via the context menu command. Manages integrated server lifecycle correctly.
- **End-to-End Flow:** Triggering "Traceform: Find Component in UI" in VS Code consistently results in the correct component being highlighted in the browser within a reasonable time frame (< 2 seconds). The system remains stable during repeated use.

### Edge Case Handling
- **No Matching Component:** If "Traceform: Find Component in UI" is triggered for a component not present in the DOM, no highlight should appear, and no errors should be thrown. Log appropriate messages (e.g., "Component X not found in current view").
...
This guide details how to set up and run all components of the Traceform system locally for development and testing purposes.
...
1.  **Clone Repository (if needed):**
    ...
    cd traceform # Navigate into the main project directory
    ```
...
2.  **Install Dependencies for All Subprojects:**
    ...
    # Note: No separate install needed for babel-plugin (installed into example-react-app from npm or used via relative path)
    ```
...
5.  **Load the Browser Extension:**
    ...
    *   Click "Load unpacked".
    *   Select the `traceform/browser-extension/dist` directory (ensure you ran the build step first).
    *   The "Traceform: UI Mapping" extension icon should appear in your browser toolbar.

6.  **Install and Run the VS Code Extension:**
    *   Open VS Code.
    *   Go to the Extensions view (Ctrl+Shift+X or Cmd+Shift+X).
    *   Search for `LucidLayer.traceform-vscode` in the Marketplace.
    *   Click "Install".
    *   The extension will activate automatically. The integrated Bridge Server will start in the background (check VS Code Output panel for "Traceform" logs if needed).
    *   Use your regular VS Code window for testing the "Traceform: Find Component in UI" command.
...
7.  **Run the Example React App (Development Server):**
    ...
    ```
    *   Open the provided URL (e.g., `http://localhost:5173`) in the browser where you loaded the unpacked Traceform: UI Mapping Browser Extension (Step 5).

### End-to-End Testing & Validation (Updated Setup)

With the VS Code extension installed from the Marketplace and the Traceform: UI Mapping Browser Extension loaded unpacked:
...
3.  Open the `traceform/example-react-app` folder in VS Code (ensure the `LucidLayer.traceform-vscode` is installed and active).
...
-   Triggering "Traceform: Find Component in UI" from your main VS Code window highlights the correct component in the browser.
-   Check the **VS Code Output Panel** for the "Traceform" channel. You should see logs indicating the Bridge Server started, client connections (from the browser extension), and `HIGHLIGHT_COMPONENT` messages being processed.
...
-   **No Highlight:**
    *   Verify the VS Code extension is active and the integrated Bridge Server started correctly (check VS Code Output Panel for "Traceform").
    ...
    *   Ensure the `@traceform/babel-plugin` is correctly configured in the `example-react-app`'s Babel/Vite config and that `data-traceform-id` attributes exist in the DOM.
    *   Check for errors in the Browser Console and VS Code Output Panel.
    *   Ensure you are using your main VS Code window.
-   **Connection Issues:**
    *   Confirm the integrated Bridge Server started on the expected port (9901) by checking the VS Code Output Panel ("Traceform" channel). If another process is using the port, the extension should log an error.
